...
import com.util.ai.screenbot.output.elements.ocr.conf.expert.VBExpertOddsOcrConf;
...
public class ExpertOddsOcrTest extends OutputHandlerTestBase {
	...
	public static void setup() {
		conf = new VBExpertOddsOcrConf();
	}

	@Test
	public void ocrTest() throws Exception {
		final File root = new File("./external/expert/odds");

		for (File f : root.listFiles()) {
			BufferedImage image = ImageIO.read(f);
			image = imageProcessor.process(image, conf);
			
			ImageIO.write(image, "png", new File("./out/expert/odds/out_" + f.getName()));
