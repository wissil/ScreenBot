...
import com.util.ai.screenbot.output.elements.ocr.conf.williamhill.VBWilliamHillOddsOcrConf;
...
public class WilliamHillOddsOcrTest extends OutputHandlerTestBase {
	...
	public static void setup() {
		conf = new VBWilliamHillOddsOcrConf();
	}

	@Test
	public void ocrTest() throws Exception {
		final File root = new File("./external/williamhill/odds");

		for (File f : root.listFiles()) {
			BufferedImage image = ImageIO.read(f);
			image = imageProcessor.process(image, conf);
			
			ImageIO.write(image, "png", new File("./out/williamhill/odds/out_" + f.getName()));
