...
public class ExpertInputBot extends AbstractInputBot {
	...
	public void checkBettingSlip(int timeout) throws InvalidBetSlipException {
		...
		if (!SikuliUtils.waitForElement(ExpertGuiConstants.EXPERT_BET_SLIP, timeout)) {
	...
	public void neutralClick() throws InvalidBetSlipException {
		try {
			SikuliUtils.clickOnElement(ExpertGuiConstants.EXPERT_BET_SLIP);
	...
	public BufferedImage takeBookmakerOddsScreenshot() throws InvalidBetSlipException {
		...
		try {
			return SikuliUtils.getImageRigtToElement(ExpertGuiConstants.EXPERT_ODDS, 25);
		} catch (GuiElementNotFoundException e) {
